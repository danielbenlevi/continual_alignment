    ...
    def __init__(
        ...
        if s_out is None:
            s_out = _random_orthonormal_matrix(
                d=self.out_features,
                m=min(self.k, max(1, self.out_features - 1)),
                device=param_device,
                dtype=param_dtype,
            )
        else:
        ...
        if s_in is None:
            s_in = _random_orthonormal_matrix(
                d=self.in_features,
                m=min(self.k, max(1, self.in_features - 1)),
                device=param_device,
                dtype=param_dtype,
            )
        else:
...
def build_safety_s_matrices(
    ...
    m_out = min(int(k), out_features - 1)
    m_in = min(int(k), in_features - 1)
...
def apply_clora_to_model(
    ...
    for full_name, module in list(model.named_modules()):
        ...
        wrapped = CLoRALinear(module, rank=rank, alpha=alpha, k=k, s_out=s_out, s_in=s_in)

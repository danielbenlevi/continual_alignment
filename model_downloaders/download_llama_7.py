...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download Llama-2-7b model locally")
    parser.add_argument(
        "--repo-id",
        default="meta-llama/Llama-2-7b-hf",
        help="Hugging Face repo id to download (HF format recommended)",
    )
    parser.add_argument(
        "--models-dir",
        default=None,
    ...
    models_dir = (
        ...
        if args.models_dir
        else default_models_dir.resolve()
    )
    if args.local_name:
        local_name = args.local_name
    elif args.repo_id.strip().lower() == "meta-llama/llama-2-7b-hf":
        # Keep compatibility with existing trainer/orchestrator local path expectations.
        local_name = "Llama-2-7b"
    else:

...
def _tokenize_example_builder(
    ...
    def generate_and_tokenize_prompt(data_point):
        user_prompt, full_prompt = _build_prompt_pair(prompter, data_point)
...
def _run_task_training(
    *,
    ...
    current_data = train_dataset_raw.shuffle(seed=42 + task_id).map(generate_and_tokenize_prompt)
    ...
    if task_id > 0:
        ...
        if memory_data_raw is not None and len(memory_data_raw) > 0:
            memory_data = memory_data_raw.shuffle(seed=42 + task_id).map(generate_and_tokenize_prompt)
    ...
    if task_id > 0 and safety_memory_raw is not None and len(safety_memory_raw) > 0:
        safety_memory_data = safety_memory_raw.shuffle(seed=4200 + task_id).map(generate_and_tokenize_prompt)
        ...
        def _run_replay_phase(
            *,
            ...
            trainer_memory = ReplayTrainer(
                ...
                use_token_kl_loss=(use_safety_heuristics and enable_safety_token_kl),
...
def train(
    ...
    performance_tasks: str = "gsm8k,sst2,mbpp",
    align_n: int = 1500,
    ...
    if len(perf_order) != 3:
        raise ValueError("performance_tasks must contain exactly 3 tasks (comma-separated).")

...
TASK_REGISTRY = {
    ...
    "samsum": dict(load_split="train", eval_type="generation"),
}
GENERATION_TASKS = {"xsum", "samsum"}
...
def _format_eval_prompt(tokenizer, prompt: str, use_chat_template: bool = False) -> str:
    return format_supervised_prompt(
        tokenizer=tokenizer,
        prompt=str(prompt),
...
def _strip_alpaca_scaffold(text: str) -> str:
    return _shared_strip_alpaca_scaffold(text)
...
def _tokenize_example_builder(
    ...
    return build_tokenize_example_fn(
        tokenizer=tokenizer,
